...
@CommandData()
public class CAPlayersCommand extends SubCommand {
    public CAPlayersCommand(String mainCommandName) {
    ...
    public boolean onPlayerCommand(Player player, String[] args) {
        ...
        try {
            ...
            PaginatedList<String> messages = new PaginatedList<>(plugin.getConfig().getStringList("CA.world.CraftAttack_Players"));
            messages.maxItemsPerPage(lgm.getCustomObject("Player.CAPlayerListCommand.MaxItemsPerPage", null, 3, false)).sort();
            ...
            player.sendMessage(lgm.getMessage("Player.CAPlayerListCommand.Header", player, false));
            ...
            player.sendMessage(lgm.getMessage("Player.CAPlayerListCommand.Footer", player, true));
    ...
    public boolean onConsoleCommand(ConsoleCommandSender sender, String[] args) {
        ...
        try {
            ...
            lgm.addPlaceholder(PlaceholderType.MESSAGE, "%max_page%", messages.getMaxPage(), false);
            sender.sendMessage(lgm.getMessage("Player.CAPlayerListCommand.Header", null, false));
            for (String playerName : messages.getPage(page)) {
                Player tempTarget = Bukkit.getPlayer(playerName);
                if (tempTarget != null) {
                    lgm.addPlaceholder(PlaceholderType.MESSAGELIST, "%count%", messages.getListOfThings().indexOf(playerName) + 1, false);
            ...
            sender.sendMessage(lgm.getMessage("Player.CAPlayerListCommand.Footer", null, true));
        } catch (NumberFormatException e) {
            Player target = Bukkit.getPlayer(args[0]);
            if (target != null) {
                lgm.addPlaceholder(PlaceholderType.MESSAGELIST, "%count%", -1, false);
    ...
    @Override
    public String info() {
        return "The CA Players List Command";
    }

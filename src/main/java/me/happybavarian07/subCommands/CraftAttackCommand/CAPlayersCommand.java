...
public class CAPlayersCommand extends SubCommand {
    ...
    public boolean onCommand(Player player, String[] args) {
        ...
        player.sendMessage("§5----------------§6Craft-§5Attack-§2Players§5-----------------");
        int count = 0;
        for (String name : plugin.getConfig().getStringList("CA.world.CraftAttack_Players")) {
            count++;
            Player target = Bukkit.getPlayer(name);
            if (target != null) {
                player.sendMessage("§3" + count + ". §2" + name + "§3:" + (target.isOnline() ? "  |  §a§lOnline" : "  |  §4§lOffline"));
